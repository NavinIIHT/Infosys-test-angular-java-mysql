...
public class ProjectAllocationAPI
{
	...
	String successMessage = environment.getProperty("API.ALLOCATION_SUCCESS")
				+ ":" + projectId;
	ResponseEntity<String> response = new ResponseEntity<String>(successMessage,
								     HttpStatus.CREATED);
	return response;
    }
	...
	ResponseEntity<List<MentorDTO>> response = new ResponseEntity<List<MentorDTO>>(mentorList,
										       HttpStatus.OK);
	return response;
    }
    ...
    public ResponseEntity<String> updateProjectMentor(@PathVariable Integer projectId,
						      @PathVariable @Min(value = 1000,
									 message = "{mentor.mentorid.invalid}") @Max(value = 9999,
														     message = "{mentor.mentorid.invalid}") Integer mentorId) throws InfyInternException
    {
	...
	ResponseEntity<String> response = new ResponseEntity<>(successMessage,
							       HttpStatus.OK);
	return response;
    }
